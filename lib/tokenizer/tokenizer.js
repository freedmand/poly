...
/**
 * @typedef {string|{type:string,open:string,close:string}|{type:string,regexp:!RegExp}}
 */
let Matcher;
...
export class Tokenizer {
  ...
  consume(pattern) {
    ...
    if (match != null && match.length > 0) {
      ...
      return new Token(match, type, lineNumber, linePosition, index);
    }
    return null;
  }
  ...
  tokenize() {
    ...
    this.consume(whitespaceRegexp);
    const token = this.attemptConsumePatternsList(
        [reserveWords, brackets, tokenRegexps]);
    if (token) {
      return token;
    } else {
