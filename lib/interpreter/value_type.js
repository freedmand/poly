...
export class ValueTypeOps {
  ...
  registerOperation(op, lambda, signature = null, returnSignature = null) {
    ...
    const value = {
      fn: (state, mappings, expectedType, ...args) => {

        if (returnSignature == null && expectedType == null) {
          ...
          const mappedReturnSignature = (expectedType == null ?
                returnSignature : expectedType).applyMappings(mappings);
  ...
  registerLiteral(op, lambda) {
    ...
    this.literal = {op, fn, typeOps: this};
  }
}
...
export class TypeTable {
  constructor() {
    ...
    /**
     * A mapping of literal type to the function that creates the appropriate
     * value type.
     * @type {!TypeIndex}
     */
    this.literalTypes = new TypeIndex();
  ...
  initialize(typeOps) {
    ...
    if (typeOps.literal !== null) {
      ...
      this.literalTypes.set(this.getType(typeOps.baseType).value.type, typeOps.literal);
    }
  }
}
