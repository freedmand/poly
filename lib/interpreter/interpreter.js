  ...
  interpret(op, state = null) {
    ...
    if (op.statements) {
      ...
      if (this.typeTable.literalOps.has(op.name)) {
        ...
        let values;
        if (op.array) {
          // If the operator is an array literal, interpret to get each value.
      ...
      let spec;
      if (op.array) {
        // Use a special array type if the operator is an array literal.
        spec = Type.array(...values);
      } else {
        spec = Type.and(...values);
      }
