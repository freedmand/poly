...
import {TypeTable, ValueType, ArrayValueType, ValueTypeOps} from '../interpreter/value_type.js';
...
export class Array extends ArrayValueType {}
export class ArrayOps extends ValueTypeOps {
  constructor() {
    super(Array);
    this.registerLiteral(ast.ArrayLiteral, (...params) => params);

    const algebraicArrayType = Type.array(new AlgebraicType(`A'`));
    this.registerOperation(ast.Add, (...x) => x[0].concat(x[1]), Type.and(
        algebraicArrayType, algebraicArrayType), algebraicArrayType);
  }
}

const polyTypeOps = [
  VariableOps, IntOps, FloatOps, StringOps, FractionOps, ArrayOps,
];
