  ...
  initializeOperators() {
    ...
    this.setOperator({type: 'variable'}, class extends Operator {
      ...
      statementHandler() {
        ...
        this.parser.advance();
        const type = this.parser.consumeUntil(ast.Type, {type: 'assign'}, true);
        // TODO: Validate that type is, in fact, a type.
        const expression = this.parser.expression();
        if (type.params.length == 0) return new this.ast(variable, expression);
