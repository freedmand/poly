  ...
  initializeOperators() {
    ...
    this.setOperator({type: 'variable'}, class extends Operator {
      ...
      statementHandler() {
        ...
        this.parser.advance();
        const type =
            this.parser.consumeUntil(ast.TypeLiteral,
                this.parser.operatorTable.generalAssignMatcher);

        // If doing a normal reassignment, the type information should not be
        // included.
        if (this.parser.operatorTable.reassignMatcher.match(
            this.parser.operator.token)) {
    ...
    this.makeBraceOperator(
        {text: '('}, {text: ')'}, ast.Group, false, ast.FunctionParams, 80);
    this.makeBraceOperator({text: '['}, {text: ']'}, ast.Array, false,
        ast.ArrayIndex, 80);
