...
import {Ast, StatementsAst} from './ast.js';
import {Token} from '../tokenizer/token.js';
import {Scope, ReadOnlyScope} from '../parser/scope.js';
...
export class OperatorTable {
   ...
   */
  constructor(statementsAst = StatementsAst) {
    /** @type {!Ast} */
    ...
    /**
     * The comma token. Can be set with setCommaToken() later.
     * @type {?Token}
     */
  ...
  /**
   * Sets the comma token to the specified token.
   * @param {!Token} token
   */
  setCommaToken(token) {
    this.commaToken = token;
    // Register a dummy operator so that encountering commas does not throw
    // parse errors.
    this.registerDummyOperator(this.commaToken);
  }
   ...
   *     statement.
   * @param {?Ast=} noCommasAst If set, returns this Ast node only if no commas
   *     are encountered.
   */
  makeBraceOperator(open, close, ast, statement = false, noCommasAst = null) {
    if (this.commaToken == null) {
      throw new Error(
          'Comma token must be set to register non-statement brace operators');
    }
    const commaToken = this.commaToken;
    if (statement) {
      this.setOperator(open, class extends Operator {
        ...
        statementHandler() {
          return this.parser.consumeUntil(this.ast, close, this.statement);
        }
      });
    } else {
      this.setOperator(open, class extends Operator {
        ...
        prefixHandler() {
          const {commas, params} = this.parser.consumeCommasUntil(
              commaToken, close);
          if (noCommasAst != null && commas == 0) {
            // If no commas are encountered and noCommasAst is set, return a new
            // instance of noCommasAst initialized with the parameters.
            return new noCommasAst(...params);
          } else {
            return new this.ast(...params);
          }
        }
      });
    }
