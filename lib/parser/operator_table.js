...
export class OperatorTable {
   ...
   *     statement.
   * @param {?Ast=} infixAst If null, then the brace operator created does not
   *     also have an infix mode. If non-null, then the operator created can be
   *     used as an infix operator, for instance, for accessing array members or
   *     calling functions.
   * @param {number=} infixPrecedence The precedence of an infix operator; only
   *     has an effect if infixAst is not null.
   */
  makeBraceOperator(open, close, ast, statement = false, infixAst = null,
      infixPrecedence = 80) {
    ...
    const newlineMatcher = this.newlineMatcher;
    if (statement) {
      // Handle statements with a separate operator class that contains a
      // statement handler.
      ...
      if (infixAst != null) {
        // Create a different class to handle infix brace operators, which
        // initializes with a binding power and provides an infix handler
        // method.
        this.setOperator(open, class extends Operator {
          constructor({token, parser}) {
            super({ast, parser, token, statement});
            ...
            /** @type {number} */
            this.leftBindingPower = infixPrecedence;
            /** @type {boolean} */
          ...
          infixHandler(left) {
            this.parser.consumeWhileExists(newlineMatcher);
            // The result is the infixAst node populated with the left param
            // followed by the params the parser consumes until a closing brace
            // is encountered.
            const result =
                this.parser.consumeUntil(infixAst, close, false, left);
            this.parser.advance(close);
            return result;
          }
        });
      } else {
        // Create a simpler operator class to handle operators that do not
        // support an infix handler.
        this.setOperator(open, class extends Operator {
          constructor({token, parser}) {
            super({ast, parser, token, statement});
          }
