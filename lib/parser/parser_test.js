...
b := 3`);
  ...
  typeSimple() {
    ...
    const parser = new Parser(tokenizer, operatorTable);
    astEquals(parser.expression(), new ast.TypeLiteral('Int'));
  },
  ...
  typeTuple() {
    ...
    astEquals(parser.expression(), new ast.Group(
      new ast.Tuple(
        new ast.TypeLiteral('Int'),
        new ast.TypeLiteral('String'),
      ),
    ));
  },

  multipleStatementsSingleLineError() {
    const tokenizer = new PolyTokenizer('a := 2 b := 3');
    const parser = new Parser(tokenizer, operatorTable);
    t.assertRaises(() => parser.parse());
  },

  arrayIndexSimple() {
    const tokenizer = new PolyTokenizer('a[2]');
    const parser = new Parser(tokenizer, operatorTable);
    astEquals(parser.expression(), new ast.ArrayIndex(
      new ast.Variable('a'),
      new ast.IntLiteral('2'),
    ));
  },
  ...
  arrayIndexTuple() {
    const tokenizer = new PolyTokenizer('a[2, 3]');
    const parser = new Parser(tokenizer, operatorTable);
    astEquals(parser.expression(), new ast.ArrayIndex(
      new ast.Variable('a'),
      new ast.Tuple(
  ...
  functionParamsSimple() {
    const tokenizer = new PolyTokenizer('a(2)');
    const parser = new Parser(tokenizer, operatorTable);
    astEquals(parser.expression(), new ast.FunctionParams(
      new ast.Variable('a'),
      new ast.IntLiteral('2'),
    ));
  },
  ...
  functionParamsTupleTrailingComma() {
    const tokenizer = new PolyTokenizer('a(2, 3,)');
    const parser = new Parser(tokenizer, operatorTable);
    astEquals(parser.expression(), new ast.FunctionParams(
      new ast.Variable('a'),
