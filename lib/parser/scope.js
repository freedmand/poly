...
export class Scope {
  ...
  constructor(parent = null) {
    ...
    this.table = new Map();
    /**
     * Whether the scope is readonly (see ReadOnlyScope).
     * @type {boolean}
     */
    this.readonly = false;
    /**
    ...
    /**
     * Stores negative internal reference numbers for reserved words, so that
     * the user counter namespace is sequential in the positive dimension.
     * @type {number}
     */
    this.negativeCounter =
        this.parent == null ? -1 : this.parent.negativeCounter;
  }
  ...
  initialize(variable, item) {
    ...
    // Set and increment the appropriate counter, depending on if the item to be
    // initialized is reserved or not.
    const reserved = item.reserved;
    if (reserved) {
      // If reserved, increment the negative counter.
      item.counter = this.getNegativeCounter();
      this.incrementNegativeCounter();
    } else {
      // Otherwise, increment the regular counter.
  ...
  toString() {
    ...
    // Maintain a token of result lines, including INDENT / UNINDENT.
    const result = [];
    let indents = 0;
    while (stack.length > 0) {
      // Pop each scope from the stack, pushing an opening brace and the
    ...
    // Convert the array of lines and indentation tokens into a cohesive string
    // representation.
    return string.toStringWithIndents(result);
  }
  ...
  getCounter() {
    if (this.parent && !this.parent.readonly) {
      this.counter = this.parent.getCounter();
    }
    return this.counter;
  }
  ...
  /**
   * Sets this negative counter to the current negative counter, defined as the
   * negative counter value at the deepest level of the parent chain.
   * @return {number} The current negative counter value.
   */
  getNegativeCounter() {
    if (this.parent && !this.parent.readonly) {
      this.negativeCounter = this.parent.getNegativeCounter();
    }
    return this.negativeCounter;
  }

  /**
   * Increments the internal negative counter for this scope and all of its
   * parents.
   */
  incrementNegativeCounter() {
    this.negativeCounter--;
    if (this.parent) this.parent.incrementNegativeCounter();
  }
}

const READONLY_ERROR = 'Read-only scope cannot be modified';

/**
 * A scope that cannot be modified or have its counter altered.
 */
export class ReadOnlyScope extends Scope {
  ...
  initialize(...unusedArgs) {
    throw new Error(READONLY_ERROR);
  }
  set(...unusedArgs) {
    throw new Error(READONLY_ERROR);
  }
  uninitialize(...unusedArgs) {
    throw new Error(READONLY_ERROR);
  }
  incrementCounter() {}
  incrementNegativeCounter() {}
