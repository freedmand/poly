...
export class Operator {
  ...
  /**
   * Returns whether the given operator matches the specified token matcher.
   ...
   */
  match(tokenMatcher) {
    const hasText = tokenMatcher.text != null;
    const hasType = tokenMatcher.type != null;
    if (hasText && hasType) {
      // If the token matcher defines both 'text' and 'type', ensure both match.
      return this.token.text == tokenMatcher.text &&
          this.token.type == tokenMatcher.type;
    }
    // Otherwise, if it has one but not the other, ensure only the one it has
    // ('text' or 'type') matches.
    if (hasText) return this.token.text == tokenMatcher.text;
    if (hasType) return this.token.type == tokenMatcher.type;
    throw new Error(
