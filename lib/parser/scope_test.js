import {Scope, ReadOnlyScope, makeVar, makeConst, makeReserved} from './scope.js';
...
t.runTests('Scope', {
  ...
  counterMultipleNestedReadOnly() {
    let topScope = new Scope();
    t.assertEquals(topScope.initialize('1', makeVar('1')), 1);
    topScope = new ReadOnlyScope(topScope);
    const bottomScopeA = new Scope(topScope);
    t.assertEquals(bottomScopeA.initialize('2', makeVar('2')), 2);
    t.assertEquals(bottomScopeA.initialize('3', makeVar('3')), 3);
    const bottomScopeB = new Scope(topScope);
    t.assertEquals(bottomScopeB.initialize('2', makeVar('2')), 2);
    t.assertEquals(bottomScopeB.initialize('3', makeVar('3')), 3);
  },

  counterNegativeSimple() {
    const scope = new Scope();
    t.assertEquals(scope.initialize('Int', makeReserved('int')), -1);
  },

  counterReservedAndNonReserved() {
    const scope = new Scope();
    t.assertEquals(scope.initialize('a', makeVar('a')), 1);
    t.assertEquals(scope.initialize('Int', makeReserved('int')), -1);
    t.assertEquals(scope.initialize('b', makeVar('b')), 2);
    t.assertEquals(scope.initialize('Float', makeReserved('float')), -2);
    t.assertEquals(scope.initialize('String', makeReserved('string')), -3);
    t.assertEquals(scope.initialize('c', makeVar('c')), 3);
  },
});
