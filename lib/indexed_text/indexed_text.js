...
export class IndexedText {
  ...
  /**
   * Returns the newline position at the specified line.
   * @param {number} line The line number at which to retrieve the newline
   *     position.
   * @return {number} The newline position.
   */
  ...
  /**
   * Returns a new IndexedText instance that is the result of applying the
   * specified delta on this instance.
   * @param {!Delta} delta The delta to apply.
   * @return {!IndexedText} The new IndexedText after applying the delta.
   */
  applyDelta(delta) {
    return new IndexedText(string.splice(
        this.text, delta.charPosition, delta.length, delta.substitution));
  }
}

/**
 * Delta represents a change in text at some position in a file.
 */
export class Delta {
  /**
   * @param {number} charPosition The character position of the start of the
   *     delta.
   * @param {number} length The length of the delta.
   * @param {string} substitution The text to substitute in the range specified.
   */
  constructor(charPosition, length, substitution) {
    /** @type {number} */
    this.charPosition = charPosition;
    /** @type {number} */
    this.length = length;
