...
export class IntermediateTranslator {
  ...
  translateAssign(op, scope) {
    ...
    if (op.has('type')) {
      ...
      if (!match) {
        throw new Error(
            `Type error: Was expecting ${expectedType}, got ${type}`);
      }
    }
  ...
  translateTypeLiteral(op, scope) {
    if (op.array) {
      return Type.array(this.translateTypeLiteral(op.params[0], scope));
    } else if (op.tuple) {
      return Type.softTuple(
          ...op.params.map((param) => this.translateTypeLiteral(param, scope)));
    } else if (op.or) {
      return Type.or(
          ...op.params.map((param) => this.translateTypeLiteral(param, scope)));
    } else if (op.group) {
      return Type.group(this.translateTypeLiteral(op.params[0], scope));
    }
    return scope.getType(op.params[0]);
  }
