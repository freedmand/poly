...
import {reduce, add, sub, mul, div, fromFloat} from './fraction.js';
...
t.runTests('Fraction', {
  ...
  reduceZero() {
    assertFractionEquals(reduce({num: 0, den: 3}), {num: 0, den: 1});
    assertFractionEquals(reduce({num: 0, den: -2}), {num: 0, den: 1});
  },
  ...
  fromFloat() {
    assertFractionEquals(fromFloat(2.3), {num: 23, den: 10});
    assertFractionEquals(fromFloat(Math.PI, 0.001), {num: 201, den: 64});
    // The golden ratio.
    assertFractionEquals(fromFloat((1 + Math.sqrt(5)) / 2, 0.001),
        {num: 55, den: 34});
    assertFractionEquals(fromFloat(0), {num: 0, den: 1});
    assertFractionEquals(fromFloat(-4), {num: -4, den: 1});
    assertFractionEquals(fromFloat(-231.325), {num: -9253, den: 40});
  },
})