...
import {AndType, OrType, PolymorphicType, Type} from './type.js';
...
t.runTests('Type', {
  ...
  getReturnTypeForArgsSimple() {
    const int = new Type('Int');
    const string = new Type('String');
    const fn = Type.function(int, string);
    t.assertTrue(Type.equals(fn.getReturnTypeForArgs(int), string));
    t.assertEquals(fn.getReturnTypeForArgs(string), null);
  },

  getReturnTypeForArgsOrComplex() {
    const int = new Type('Int');
    const string = new Type('String');
    const float = new Type('Float');
    const fn = Type.or(
      Type.function([int, int], string),
      Type.function([int, string], float),
      Type.function(float, int),
    );
    t.assertTrue(
        Type.equals(fn.getReturnTypeForArgs(Type.and(int, int)), string));
    t.assertTrue(
        Type.equals(fn.getReturnTypeForArgs(Type.and(int, string)), float));
    t.assertTrue(Type.equals(fn.getReturnTypeForArgs(float), int));

    t.assertEquals(fn.getReturnTypeForArgs(int), null);
    t.assertEquals(fn.getReturnTypeForArgs(string), null);
    t.assertEquals(fn.getReturnTypeForArgs(Type.and(int, float)), null);
    t.assertEquals(fn.getReturnTypeForArgs(Type.and(float, int)), null);
    t.assertEquals(fn.getReturnTypeForArgs(Type.and(float, float)), null);
    t.assertEquals(fn.getReturnTypeForArgs(Type.and(float, string)), null);
    t.assertEquals(fn.getReturnTypeForArgs(Type.and(string, int)), null);
    t.assertEquals(fn.getReturnTypeForArgs(Type.and(string, float)), null);
    t.assertEquals(fn.getReturnTypeForArgs(Type.and(string, string)), null);
  },

  orSameType() {
    const int = new Type('Int');
    t.assertTrue(Type.equals(Type.or(int, int), int));
  },

  orSameTypeComplex() {
    const int = new Type('Int');
    const string = new Type('String');
    t.assertTrue(Type.equals(Type.or(int, string, int), Type.or(int, string)));
  },
  ...
  orMultipleFlattened() {
    ...
    const float = new Type('Float');
    const string = new Type('String');
    const intOrFloat = Type.or(int, float);
    const intOrFloatOrString = Type.or(int, float, string);
    t.assertTrue(Type.equals(Type.or(intOrFloat, string), intOrFloatOrString));
  },

  orMultipleFlattenedComplex() {
    const int = new Type('Int');
    const float = new Type('Float');
    const string = new Type('String');
    const intOrFloat = Type.or(int, float);
    const intOrFloatOrString = Type.or(int, float, string);
    t.assertTrue(
        Type.equals(Type.or(intOrFloat, string, int), intOrFloatOrString));
  },

  softTupleSimple() {
    // Int,Int,Int
    const int = new Type('Int');
    const softTuple = Type.softTuple(Type.softTuple(int, int), int);
    t.assertTrue(Type.equals(softTuple, Type.and(int, int, int)));
  },

  softTupleSimpleOtherOrder() {
    // Int,Int,Int
    const int = new Type('Int');
    const softTuple = Type.softTuple(int, Type.softTuple(int, int));
    t.assertTrue(Type.equals(softTuple, Type.and(int, int, int)));
  },
  ...
  softTupleBrokenLeft() {
    // (Int,Int),Int
    const int = new Type('Int');
    const softTuple = Type.softTuple(Type.group(Type.softTuple(int, int)), int);
    t.assertTrue(Type.equals(softTuple, Type.and(Type.and(int, int), int)));
  },

  softTupleBrokenRight() {
    // Int,(Int,Int)
    const int = new Type('Int');
    const softTuple = Type.softTuple(int, Type.group(Type.softTuple(int, int)));
    t.assertTrue(Type.equals(softTuple, Type.and(int, Type.and(int, int))));
  },
});
