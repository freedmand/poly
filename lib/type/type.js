...
/**
 * Removes empty types from a list of types.
 * @param {!Array<!Type>} types The types to filter.
 * @return {!Array<!Type>} The types without any empty types.
 */
function filterEmpty(types) {
  return types.filter((type) => !type.empty);
}
...
export class Type {
  ...
  constructor(type, childType=null) {
    ...
    /**
     * Whether this type is an empty type.
     * @param {boolean}
     */
    this.empty = false;
  }
   ...
   * @param {?Type} type2 The second type, or null if the tuple only contains a
   *     single type.
   * @return {!Type} The resulting tuple type.
   */
  static softTuple(type1, type2 = null) {
    /**
    ...
    // Conjoin the types from type1 and type2.
  ...
  /**
   * Constructs an array type out of the specified type, returning an array type
   * containing all the subtypes if a tuple (`and` type) is passed in, or an
   * array type just containing a single type if something else is passed in.
   * @param {!Type} type The type to pass in that corresponds with what the Ast
   *     representation of the array looks like.
   */
  static softArray(type) {
    if (type.and) {
      // Break apart all subtypes if an `and` type is passed in.
      return Type.array(...type.types);
    }
   ...
   * @return {!Type} The resulting function type.
   */
   ...
   * @return {!Type} The resulting empty type.
   */
  static empty() {
...
/**
 * GenericType represents an Ast type that is not fully populated with type
 * information about children nodes. The specification is by default set to its
 * class name.
 */
...
export class AndType extends Type {
  ...
  constructor(...types) {
    if (types.length == 0) {
      ...
      // Construct a specification of the form `type1,...,typeN`.
      let signature = types.map((x) => extractSpec(x)).join(',');
      // Add a trailing comma for single-element tuple types.
      if (types.length == 1) signature += ',';
      super(signature);
    }
    /**
...
/**
 * An empty type that is a placeholder between a trailing comma and closing
 * brace.
 */
export class EmptyType extends Type {
  ...
  matchInternal(type) {
    throw new Error('This should never be called on empty type');
  }
