...
export class ArrayType extends Type {
  ...
  constructor(...types) {
    ...
    this.specification = `[${extractSpec(type)}]`;
    /**
  ...
  updateType(type) {
    this.type = type;
    // The specification in this case does not need to be enclosed in square
    // brackets, as the type being assigned should already be an array type.
    this.specification = `${extractSpec(this.type)}`;
  }
  ...
  match(type) {
    ...
    return this.type.match(type.type);
  }
}
